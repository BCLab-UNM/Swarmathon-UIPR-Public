...
// result.pd.cmdAngular = getDirection() * (2.5 * e + 0.01 * diffE) + K_angular * (angleMin - getDirection()*M_PI_2);
...
void ObstacleController::follow_Wall()
{
  ...
  if (right < 0.3 || center < 0.3 || left < 0.3)
  {
    ...
    result.pd.cmdAngular = getDirection() * (2.3 * e + 1 * diffE);
    cout << "Angular Vel = " << result.pd.cmdAngular << endl; //Debug
    result.pd.cmdVel = 0.2;
  }
  distRead.clear();
